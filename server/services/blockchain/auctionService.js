...
async function initiateAuction(tokenId, startingBid, bidIncrement, duration) {
  ...
  const formattedBidIncrement = ethers.utils.parseEther(
    bidIncrement.toString()
  );

  // Start a new auction for a creator token
  const tx = await contractInstance.initiateAuction(
    tokenId,
    formattedStartingBid,
    formattedBidIncrement,
    duration
  );
  ...
  console.log(
    `Auction initiated for token ID ${tokenId} with transaction receipt:`,
    receipt
  );
}
...
async function placeBid(auctionId, bidAmount) {
  ...
  console.log(
    `Bid placed in auction ID ${auctionId} with transaction receipt:`,
    receipt
  );
}
...
async function finalizeAuction(auctionId) {
  ...
  console.log(
    `Auction ID ${auctionId} finalized with transaction receipt:`,
    receipt
  );
}
