...
describe('DiamondTest', async function () {
  ...
  before(async function () {
    this.timeout(30000); // Extend timeout if deployment takes longer
    diamondAddress = await deployDiamond();
    assert.ok(diamondAddress, 'Failed to deploy diamond contract.');
  ...
  it('should add test1 functions and validate selectors', async () => {
    ...
    assert.equal(receipt.status, 1, 'Diamond upgrade should succeed.');
    result = await diamondLoupeFacet.facetFunctionSelectors(test1Facet.address);
    assert.sameMembers(result, selectors, 'Selectors should match after adding test1 functions.');
  });
