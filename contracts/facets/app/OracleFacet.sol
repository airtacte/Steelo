...
contract OracleFacet is ChainlinkClient, AccessControlFacet {
    ...
    function requestVolumeData(
        ...
        bytes32 _jobId,
        address _oracle
    ) public onlyRole(accessControl.ADMIN_ROLE()) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ...
        req.add(
            "get",
            "https://us-central1-steelo-47.cloudfunctions.net/functionName"
        ); // Corrected method
        req.add("path", "volume");
    ...
    function getSteezTransactionCount(
        uint256 creatorId
    ...
    function getSteeloCurrentPrice(
        uint256 creatorId
    ) external view returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
