...
contract GovernanceFacet is AccessControlFacet {
    ...
    function hasMetQuorum(uint256 proposalId) private view returns (bool) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256 totalVotes = ds.sips[proposalId].voteCountFor +
            ds.sips[proposalId].voteCountAgainst;
    ...
    function getVoterFor(address voter) public view returns (address) {
        return
            voteDelegations[voter] == address(0)
                ? voter
                : voteDelegations[voter];
    }
    ...
    function _propose(
        ...
        string memory benefits,
        string memory metadataURI
    ) public onlyRole(accessControl.INVESTOR_ROLE()) returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
    ...
    function cancelProposal(
        uint256 proposalId
    ) public onlyRole(accessControl.MODERATOR_ROLE()) {
