...
contract STEEZFacet is ERC1155Upgradeable, AccessControlFacet {
    ...
    // FUNCTIONS
    function initialize(string memory _baseURI) public onlyRole(accessControl.EXECUTIVE_ROLE()) initializer {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
     ...
     */
    function createSteez(bytes memory data) public onlyRole(accessControl.USER_ROLE()) nonReentrant (msg.sender) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
    ...
    // Pre-order function
    function preOrder(uint256 creatorId, uint256 amount) public payable onlyRole(accessControl.CREATOR_ROLE()) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
    ...
    // Launch function
    function launch(uint256 creatorId, uint256 amount) internal onlyRole(accessControl.CREATOR_ROLE()) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        BazaarFacet bazaar = BazaarFacet(ds.bazaarFacetAddress);
        FeesFacet fees = FeesFacet(ds.feesFacetAddress);
    ...
    function anniversary(uint256 creatorId, uint256 amount) external payable onlyRole(accessControl.CREATOR_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        BazaarFacet bazaar = BazaarFacet(ds.bazaarFacetAddress);
        FeesFacet fees = FeesFacet(ds.feesFacetAddress);
    ...
    function mintSteez(address to, uint256 creatorId, uint256 amount, bytes memory data) internal {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
    ...
    function transferSteez(uint256 creatorId, uint256 steezId, uint256 amount, address from, address to) external onlyRole(accessControl.INVESTOR_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        FeesFacet fees = FeesFacet(ds.feesFacetAddress);
