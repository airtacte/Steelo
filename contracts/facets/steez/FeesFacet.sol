...
contract FeesFacet is OwnableUpgradeable, ReentrancyGuardUpgradeable, Initializable {
    ...
    function initialize(address owner) public onlyExecutive initializer {
        ...
        function payRoyalties(uint256 creatorId, uint256 amount, address from, address to, bytes memory data) external payable nonReentrant {
            ...
            distributeRoyalties(creatorId, amount, creatorFee, steeloFee, communityFee, from, to);
            refundExcessPayment(amount);

            bool success = distributeRoyalties(creatorId, amount, creatorFee, steeloFee, communityFee, from, to);
            if (success) {
        ...
        function calculateCreatorFee(uint256 creatorId, uint256 amount) internal view returns (uint256) {
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
            ...
            } else if (totalSupply > ds.constants.PRE_ORDER_SUPPLY && totalSupply <= (ds.constants.PRE_ORDER_SUPPLY + ds.constants.LAUNCH_SUPPLY)) {
        ...
        function calculateSteeloFee(uint256 creatorId, uint256 amount) internal view returns (uint256) {
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ...
        function calculateCommunityFee(uint256 creatorId, uint256 amount) internal view returns (uint256) {
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ...
        function distributeRoyalties(uint256 creatorId, uint256 amount, address from, address to) internal nonReentrant {
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
            ...
            (success, ) = payable(ds.creators[creatorId].profileAddress).call{value: creatorFee}("");
            ...
            emit RoyaltiesDistributed(from, to, creatorId, amount, creatorFee, steeloFee, communityFee, "");
        }

        function distributeCommunityFee(uint256 creatorId, uint256 communityFee) internal nonReentrant {
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
            ...
            for (uint256 i = 0; i < ds.steez[creatorId].investors.length; i++) {
                address payable investorAddress = payable(ds.steez[creatorId].investors[i].investorAddress);
                uint256 investorBalance = ds.steez[creatorId].investors[i].balance;
                uint256 investorShare = (communityFee * investorBalance) / totalSupply;
                ...
                if (!success) {
                    ...
                    queueFailedRoyaltyPayment(creatorId, investorShare, investorAddress);
                }
        ...
        function viewRoyalties(address user, uint256 creatorId) public view returns (uint256 userShare, uint256 userRoyalty) {
            ...
            userShare = (ds.royalty.totalRoyalties * ds.steez[creatorId].balance) / ds.steez[creatorId].totalSupply;

            // Calculate user's total royalty
            uint256 userSharePercentage = (ds.steez[creatorId].balance * 10000) / ds.steez[creatorId].totalSupply;
