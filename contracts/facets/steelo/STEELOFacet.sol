...
contract STEELOFacet is ERC20Upgradeable, ChainlinkClient, AccessControlFacet {
    ...
    function steeloTGE(
        ...
        require(
            !ds.tgeExecuted,
            "STEELOFacet: steeloTGE can only be executed once"
        );
        require(
            ds.totalTransactionCount == 0,
            "STEELOFacet: TransactionCount must be equal to 0"
        );
        require(
            ds.steeloCurrentPrice > 0,
        ...
        uint256 communityAmount = (ds.constants.TGE_AMOUNT *
            ds.constants.communityTGE) / 100;
        uint256 foundersAmount = (ds.constants.TGE_AMOUNT *
            ds.constants.foundersTGE) / 100;
        uint256 earlyInvestorsAmount = (ds.constants.TGE_AMOUNT *
            ds.constants.earlyInvestorsTGE) / 100;
        uint256 treasuryAmount = (ds.constants.TGE_AMOUNT *
            ds.constants.trasuryTGE) / 100;
    ...
    function tokenTransfer(
        ...
        require(
            recipient != address(0),
            "STEELOFacet: Cannot transfer to the zero address"
        );
        require(
            amount <= balanceOf(msg.sender),
            "STEELOFacet: Not enough tokens"
        );
        ...
        nonReentrant
    {
        ...
        require(
            totalSupply() > ds.constants.TGE_AMOUNT,
            "STEELOFacet: steeloMint can only be called after the TGE"
        );
        require(
            ds.totalTransactionCount > 0,
            "STEELOFacet: ds.totalTransactionCount must be greater than 0"
        );
        require(
            ds.steeloCurrentPrice > 0,
            "STEELOFacet: steeloCurrentPrice must be greater than 0"
        ...
        ds.mintAmount = calculateMintAmount(
            ds.totalTransactionCount,
            ds.steeloCurrentPrice
        );

        // Calculate distribution amounts using ds references for percentages
        uint256 treasuryAmount = (ds.mintAmount * ds.constants.treasuryMint) /
            100;
        uint256 liquidityProvidersAmount = (ds.mintAmount *
            ds.constants.liquidityProvidersMint) / 100;
        uint256 ecosystemProvidersAmount = (ds.mintAmount *
            ds.constants.ecosystemProvidersMint) / 100;
    ...
    function calculateMintAmount(
        ...
        if (ds.steeloCurrentPrice >= ds.constants.pMax) {
            adjustmentFactor +=
                ((ds.steeloCurrentPrice - ds.constants.pMax) *
                    ds.constants.alpha) /
                100;
        } else if (ds.steeloCurrentPrice <= ds.constants.pMin) {
            adjustmentFactor -=
                ((ds.constants.pMin - ds.steeloCurrentPrice) *
                    ds.constants.beta) /
                100;
        } else {
    ...
    function calculateBurnAmount(
        ...
        if (ds.totalTransactionCount >= 0) {
            ds.burnAmount =
                (transactionValue * ds.constants.FEE_RATE * ds.burnRate) /
                1e4 /
                1e4; // in basis of 1/100 of a percent
    ...
    function adjustMintRate(
        uint256 _newMintRate
    ) external onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        require(
            _newMintRate >= ds.constants.MIN_MINT_RATE &&
                _newMintRate <= ds.constants.MAX_MINT_RATE,
            "STEELOFacet: Invalid mint rate"
        );
    ...
    function adjustBurnRate(
        uint256 _newBurnRate
    ) external onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        require(
            _newBurnRate >= ds.constants.MIN_BURN_RATE &&
                _newBurnRate <= ds.constants.MAX_BURN_RATE,
            "STEELOFacet: Invalid burn rate"
        );
    ...
    function burnTokens() private {
        ...
        ds.burnAmount =
            (((ds.steeloCurrentPrice * ds.constants.FEE_RATE) / 1000) *
                ds.burnRate) /
            100;

        require(
            treasuryBalance >= ds.burnAmount,
            "STEELOFacet: Not enough tokens to burn"
        );
        require(
            ds.burnAmount > 0,
            "STEELOFacet: Burn amount must be greater than 0"
        );
    ...
    function updateParameters() external {
        ...
        require(
            ds.steeloCurrentPrice >= ds.constants.pMin &&
                ds.steeloCurrentPrice <= ds.constants.pMax,
            "STEELOFacet: Price out of allowed range"
        );
    ...
    function fulfill(
        ...
        require(
            ds.totalTransactionCount > 0,
            "STEELOFacet: Invalid transaction count"
        );
