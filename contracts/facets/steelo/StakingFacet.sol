...
contract StakingFacet is AccessControlFacet {
        ...
        if (ds.stakes[msg.sender] == 0) {
            for (uint i = 0; i < ds.stakeholders.length; i++) {
                if (ds.stakeholders[i] == msg.sender) {
                    ds.stakeholders[i] = ds.stakeholders[
                        ds.stakeholders.length - 1
                    ];
                    ds.stakeholders.pop();
    ...
    function calculateReward(
        address stakeholder
    ) internal view onlyRole(accessControl.STAKER_ROLE()) returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ...
        } else if (ds.stakeDuration >= 90 days) {
            // 3-Month Duration
            yieldRate = 49; // Representing 4.9%
        } else if (ds.stakeDuration >= 30 days) {
            // 1-Month Duration
    ...
    function distributeRewards()
        external
        onlyRole(accessControl.STAKER_ROLE())
        nonReentrant
    {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        STEELOFacet steeloFacet = STEELOFacet(ds.steeloFacetAddress);
        ...
        require(
            ds.totalRewardPool > 0,
            "No rewards available for distribution"
        );
    ...
    function stakeholderStatus(
        address _user
    ) public view onlyRole(accessControl.STAKER_ROLE()) returns (bool) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
    ...
    function stakeAmount(
        address _user
    ) external view onlyRole(accessControl.STAKER_ROLE()) returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
