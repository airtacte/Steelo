...
contract SIPFacet is AccessControlFacet {
    ...
    event SIPCreated(uint256 indexed id, string description, string proposalType, address indexed proposer, uint256 startTime, uint256 endTime);
    ...
    function initialize(address _treasury, address _oracle, string memory _jobId, uint256 _fee, address _linkToken) public onlyRole(accessControl.EXECUTIVE_ROLE()) initializer {
    ...
    function getProposerRole(address _proposer) internal view returns (string memory) {
        if (accessControl.hasRole(accessControl.CREATOR_ROLE(), _proposer)) {
            return "creator";
        } else if (accessControl.hasRole(accessControl.USER_ROLE(), _proposer)) {
            return "community";
        } else if (accessControl.hasRole(accessControl.EXECUTIVE_ROLE(), _proposer)) {
            return "platform";
        }
        return "unknown";
    }
    ...
    function createSIP(string memory _title, string memory _description, uint256 _startTime, uint256 _endTime, address _proposer) external onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
        ...
        emit SIPCreated(sipId, _description, "YourProposalTypeHere", _proposer, _startTime, _endTime);
    }

    // In the voteOnSIP function, double the vote count of the proposer's role
    function voteOnSIP(uint256 _sipId, bool _support) external onlyRole(accessControl.STAKER_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(_sipId < ds.lastSipId, "SIP does not exist");
        require(!ds.sips[_sipId].votes[msg.sender], "Already voted");
        require(block.timestamp >= ds.sips[_sipId].startTime && block.timestamp <= ds.sips[_sipId].endTime, "Voting is not active");
        ...
        } else if (accessControl.hasRole(accessControl.USER_ROLE(), msg.sender)) {
            if (_support) {
                ds.sips[_sipId].voteCountForCommunity += 1;
            } else {
                ds.sips[_sipId].voteCountAgainstCommunity += 1;
            }
        } else if (accessControl.hasRole(accessControl.EXECUTIVE_ROLE(), msg.sender)) {
            if (_support) {
        ...
        uint256 creatorVote = ds.sips[_sipId].voteCountForCreator > ds.sips[_sipId].voteCountAgainstCreator ? (keccak256(abi.encodePacked(ds.sips[_sipId].proposerRole)) == keccak256(abi.encodePacked("creator")) ? 2 : 1) : 0;
        uint256 communityVote = ds.sips[_sipId].voteCountForCommunity > ds.sips[_sipId].voteCountAgainstCommunity ? (keccak256(abi.encodePacked(ds.sips[_sipId].proposerRole)) == keccak256(abi.encodePacked("community")) ? 2 : 1) : 0;
        uint256 steeloVote = ds.sips[_sipId].voteCountForSteelo > ds.sips[_sipId].voteCountAgainstSteelo ? (keccak256(abi.encodePacked(ds.sips[_sipId].proposerRole)) == keccak256(abi.encodePacked("steelo")) ? 2 : 1) : 0;
    ...
    function executeSIP(uint256 _sipId) public onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(_sipId < ds.lastSipId, "SIP does not exist");
    ...
    function viewSIP(uint256 _sipId) public view returns (
        uint256 sipId,
        string memory title,
    ...
    function getVoteCount(uint256 _sipId) public view returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(ds.sips[_sipId].sipId != 0, "SIP does not exist");
        return ds.sips[_sipId].voteCountForSteelo + ds.sips[_sipId].voteCountForCreator + ds.sips[_sipId].voteCountForCommunity;
    }
    ...
    function depositToTreasury(uint256 _amount) external onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
    ...
    function withdrawFromTreasury(uint256 _amount) external onlyRole(accessControl.ADMIN_ROLE()) nonReentrant {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(ds.treasury[address(STEELOFacet(ds.steeloFacetAddress))] >= _amount, "Insufficient funds in treasury");
        ds.treasury[address(STEELOFacet(ds.steeloFacetAddress))] -= _amount;
